...
<template>
  <div class="flex justify-content-between mb-3">
    <h5>AI Боти</h5>
    <Button v-if="bots.length > 0" label="Додати" @click="addBot" type="button" class="py-3 px-5"></Button>
  </div>
  <DataView v-if="bots.length > 0" :value="bots" :layout="layout" :paginator="true" :rows="9">
    ...
    <template #list="slotProps">
      <div class="grid grid-nogutter">
        <router-link :to="{ 'name':'channels' }" v-for="(channel, index) in slotProps.items" :key="index"
                     class="col-12">
    ...
    <template #grid="slotProps">
      <div class="grid grid-nogutter">
        <router-link :to="{ name: 'ai-bots-view', params: { id: bot.id} }"
                     v-for="(bot, index) in slotProps.items" :key="index" class="col-12 sm:col-6 md:col-4 p-2">
          <div class="p-4 border-1 surface-border surface-card border-round flex flex-column">
            <div class="pt-2">
              <div class="text-md font-medium text-400 my-1">{{ bot.id }}</div>
              <div class="text-lg font-medium text-900 my-1">{{ bot.topic }}</div>
              <Badge v-for="keyword in bot.keywords" :value="keyword" class="mr-2"></Badge>
            </div>
          </div>
        </router-link>
      </div>
  ...
  <div v-if="bots.length === 0" class="grid">
    <div class="col-12">
      <div class="card text-center">
        <h4>Проект не має АІ ботів</h4>
        <Button label="Створити АІ бота" @click="addBot" type="button" class="py-3 px-5"></Button>
      </div>
    </div>
  </div>
</template>
